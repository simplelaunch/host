...
function essence_woocommerce_theme_notice() {
	...
	$notice_html = sprintf( __( 'Please install and activate <a href="https://wordpress.org/plugins/genesis-connect-woocommerce/" target="_blank">Genesis Connect for WooCommerce</a> to <strong>enable WooCommerce support for %s</strong>.', 'essence-pro' ), esc_html( CHILD_THEME_NAME ) );
	...
	if ( current_user_can( 'install_plugins' ) ) {
		...
		$notice_html = sprintf( __( 'Please %1$s to <strong>enable WooCommerce support for %2$s</strong>.', 'essence-pro' ), $install_link, esc_html( CHILD_THEME_NAME ) );
	}
...
function essence_notice_script() {
	...
	wp_enqueue_script( 'essence_notice_script', get_stylesheet_directory_uri() . '/lib/woocommerce/js/notice-update.js', array( 'jquery' ), '1.0', true );
...
function essence_reset_woocommerce_notice() {
	...
	$args = array(
		'meta_key'   => $wpdb->prefix . 'essence_woocommerce_message_dismissed',
		'meta_value' => 1,
	);
