...
function magazine_woocommerce_theme_notice() {
	...
	$notice_html = sprintf( __( 'Please install and activate <a href="https://wordpress.org/plugins/genesis-connect-woocommerce/" target="_blank">Genesis Connect for WooCommerce</a> to <strong>enable WooCommerce support for %s</strong>.', 'magazine-pro' ), esc_html( CHILD_THEME_NAME ) );
	...
	if ( current_user_can( 'install_plugins' ) ) {
		...
		$notice_html = sprintf( __( 'Please %s to <strong>enable WooCommerce support for %s</strong>.', 'magazine-pro' ), $install_link, esc_html( CHILD_THEME_NAME ) );
	}

	echo '<div class="notice notice-info is-dismissible magazine-woocommerce-notice"><p>' . $notice_html . '</p></div>';
...
function magazine_notice_script() {
	wp_enqueue_script( 'magazine_notice_script', get_stylesheet_directory_uri() . '/lib/woocommerce/js/notice-update.js', array( 'jquery' ), '1.0', true  );
}
...
function magazine_reset_woocommerce_notice() {
	...
	$args = array(
		'meta_key'   => $wpdb->prefix . 'magazine_woocommerce_message_dismissed',
		'meta_value' => 1,
	);
 ...
 */
function magazine_reset_woocommerce_notice_on_deactivation( $plugin, $network_activation ) {
